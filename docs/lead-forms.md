...
5. Choose the lead source. It is the same list as a new enquiry in Reach:
   - Recovery.com
   - Returning Client
   - Ex-resident
   - Google.com
   ...
   - Referrer
   - Personal Contact
6. For Recovery Coach, Referrer, and Personal Contact, fill in **Who?** (the coach, referrer, or contact). That name is stored on every enquiry this form opens, in the same Who field as a staff enquiry.
7. Add a campaign label if this placement is a specific push (for example `Spring 2026`). The monthly report groups by that label.
8. Save. The form page shows the hosted URL, an iframe snippet, and a script snippet.
...
- Optional Cloudflare Turnstile. Set both variables and the checkbox appears on the form. If the secret is set and the check fails, the enquiry is not saved. If either variable is unset, the hosted form and embed still work with the honeypot and the rate limit.
...
If `META_LEADGEN_VERIFY_TOKEN` is unset, the handshake answers 503 and Meta cannot subscribe. If `META_APP_SECRET` is unset, deliveries are refused (503) and nothing is saved. A bad signature is rejected. Without the page token, Reach still files a New Enquiry for the lead id and source **Meta ads**, and the card says the contact details were not fetched. With the token, Reach calls `GET https://graph.facebook.com/<version>/<leadgen_id>` and maps name, email, phone, country, and any house, resident, or message answers. The hosted form does not use these variables.
...
Reach accepts the key in the JSON body (`google_key`, which Google’s lead form webhook sends), the query `?google_key=`, or the header `X-Google-Ads-Key`. If `GOOGLE_ADS_WEBHOOK_KEY` is unset, the endpoint refuses the delivery (503) and does not save a lead. A wrong key is rejected. The hosted form does not use this key.
...
This branch is rebased onto the enquiry-field and live bed-board work.

The form builder uses the same `LEAD_SOURCES` as a new enquiry, including **Who?** for Recovery Coach, Referrer, and Personal Contact. A submission writes that source, Who, caller name, and resident name onto the enquiry. Attribution (campaign, country, consent, UTM parameters, click ids) stays on `enquiry_intake`.

The lead-form migration only creates its own tables and adds `lead_source_who` when that column is missing. It does not update or delete people, rooms, or bed assignments.

`npm run db:seed` does not insert lead forms or lead-form enquiries. Production already runs that command on deploy, so demo forms must not be added there.
